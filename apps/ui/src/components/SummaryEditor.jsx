...
import { Copy, Check, Plus, Trash2, RotateCw, CircleAlert, User, Calendar } from 'lucide-react';
...
export function SummaryEditor({ meeting, onUpdateMeeting, onRegenerateSummary, isGenerating = false }) {
    ...
    return (
        <Tabs value={activeTab} onValueChange={setActiveTab} asChild>
            <section aria-label="Meeting notes" className="flex h-full flex-col overflow-hidden rounded-xl border">
                <div className="flex flex-col gap-4 p-4 hairline-bottom sm:p-4">
                    ...
                    <p className="flex items-start gap-1 text-footnote text-muted-foreground">
                        <CircleAlert className="mt-[1px] size-4 shrink-0" aria-hidden="true" />
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-4">
                    ...
                    <TabsContent value="decisions" className="space-y-4">
                        ...
                        <ul className="space-y-1">
                            {decisions.length === 0 ? (
                                <li className="rounded-lg bg-muted px-4 py-2 text-callout text-muted-foreground">
                                    No decisions recorded. Add the first one below.
                                </li>
                            ) : (
                                decisions.map((decision, idx) => (
                                    <li key={idx} className="group flex items-start justify-between gap-4 rounded-lg bg-muted px-4 py-2">
                                        <div className="flex items-start gap-2">
                                            <Check className="mt-[2px] size-4 shrink-0 text-success" aria-hidden="true" />
                    ...
                    <TabsContent value="actions" className="space-y-4">
                        ...
                        <ul className="space-y-1">
                            {actionItems.length === 0 ? (
                                <li className="rounded-lg bg-muted px-4 py-2 text-callout text-muted-foreground">No action items yet.</li>
                            ) : (
                                actionItems.map((item, idx) => {
                                    ...
                                    return (
                                        <li key={id} className="group flex items-start justify-between gap-4 rounded-lg bg-muted px-4 py-2">
                                            <div className="flex min-w-0 flex-1 items-start gap-2">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <div className="mt-1 flex flex-wrap items-center gap-4 text-footnote text-muted-foreground">
                        ...
                        <form onSubmit={handleAddAction} className="flex flex-col gap-2 rounded-lg bg-muted p-2 sm:flex-row">
                            <Input
                                value={newActionTask}
